...
`{"extends": ["github>go-viture/.github"]}`.
...
hardware to `3fdac6780b`), and a repository without a loong64 lane sees
nothing wrong with the bump. A library that breaks is one library; a toolchain
that breaks is every architecture at once.
